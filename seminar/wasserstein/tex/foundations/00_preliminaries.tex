...
\begin{remark}{記法 $\eta_n\weakto\eta$}{meas-weak-arrow-notation}
  Villani (2009) p.~43 や Billingsley \emph{Convergence of Probability Measures}
  (2nd ed., 1999) では，弱収束を $\eta_n\Rightarrow\eta$ と書く．
  しかし $\Rightarrow$ は含意（「ならば」）にも用いる記号であり，
  本資料では両者が同じ文中に現れる箇所がある．
  そこで弱収束には，返しが上側だけの矢印
  \[
    \eta_n\weakto\eta
  \]
  を用いる（関数解析で弱収束に用いられる記号と同じ）．
  $\Rightarrow$ は含意のみに使う．
\end{remark}
