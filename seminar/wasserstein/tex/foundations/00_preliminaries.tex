...
\begin{definition}{確率測度の弱位相と弱収束（Villani p.~43）}{meas-weak-convergence}
  距離空間 $\mathcal{X}$ に対し，$\Prob(\mathcal{X})$ 上の
  \textbf{弱位相}とは，すべての $f\in C_b(\mathcal{X})$ について写像
  \[
    \Prob(\mathcal{X})\ni\eta
    \longmapsto
    \int_{\mathcal{X}}f\,\d\eta\in\R
  \]
  を連続にする最も粗い位相（開集合が最も少ない位相）をいう．
  この位相に関して確率測度列
  $(\eta_n)$ が $\eta\in\Prob(\mathcal{X})$ に収束することは，
  \[
    ...
    \qquad(\forall f\in C_b(\mathcal{X}))
  \]
  と同値である．これを $\eta_n$ が $\eta$ に\textbf{弱収束}するといい，
  $\eta_n\Rightarrow\eta$ と書く．
\end{definition}
...
\begin{proof}
  座標写像 $(x,y)\mapsto x$ は連続だから，以下の被積分関数は可測である．
  ...
  \]
  積分の線形性により非負単関数でも成り立ち，
  非負単関数列による近似と単調収束定理
  （定理~\ref{thm:meas-mct}）により非負可測関数へ拡張される．
  $f$ が $\mu$-可積分ならば，まず非負の場合の結果を $\abs{f}$ に適用して
  \[
    \int_{\mathcal{X}\times\mathcal{Y}}\abs{f(x)}\,\d\pi(x,y)
    =\int_{\mathcal{X}}\abs{f}\,\d\mu<\infty
  \]
  を得てから $f=f^+-f^-$ と分解すれば積分公式が従う．
  最後に非負可測関数 $\abs{f}^p$ に積分公式を適用して $p$ 乗根を取れば，
  $L^p$ ノルムの等式を得る．
\end{proof}
