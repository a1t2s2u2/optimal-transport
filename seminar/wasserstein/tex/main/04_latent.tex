...
\begin{proof}
  Tonelli の定理（定理~\ref{thm:meas-product}）より
  $\bar\rho$ は可測で，
  ...
  と言い換えられる．非負可測な $g$ に Tonelli の定理を
  適用して積分順序を交換すれば
  \[
    \int_\Theta H_\nu\bigl(\rho(\theta,\cdot)\,\nu\bigr)\,\d P
...
\begin{corollary}{ELBO の正則化項は $W_2$ を制御する}{lt-elbo-w2}
  ...
  Tonelli の定理から従うので，$Q$ は Markov 核である）．
...
\begin{proof}
  ...
  $\mu_i$ である（矩形集合上で直接確かめられ，
  一致定理~\ref{thm:meas-uniqueness}による）．
  よって周辺積分公式（補題~\ref{lem:meas-marginal-integral}）より
  \[
  ...
  $\pi\in\Couplings(\mu,\nu)$ を任意に取り，
  $(x_i,y_i)$ 成分の周辺分布
  $\pi_i\in\Prob(\mathcal{X}_i\times\mathcal{X}_i)$ を
  \[
    \pi_i(E)
    :=\pi\bigl(\{(x,y)\in\mathcal{X}\times\mathcal{X}
      \mid (x_i,y_i)\in E\}\bigr)
    \qquad(E\in\Borel(\mathcal{X}_i\times\mathcal{X}_i))
  \]
  とする．任意の $A_i,B_i\in\Borel(\mathcal{X}_i)$ に対し
  \[
    \begin{aligned}
      \pi_i(A_i\times\mathcal{X}_i)
      &=\mu\bigl(\{x\in\mathcal{X}\mid x_i\in A_i\}\bigr)
      =\mu_i(A_i),\\
      \pi_i(\mathcal{X}_i\times B_i)
      &=\nu\bigl(\{y\in\mathcal{X}\mid y_i\in B_i\}\bigr)
      =\nu_i(B_i).
    \end{aligned}
  \]
  したがって $\pi_i\in\Couplings(\mu_i,\nu_i)$ である．よって
  \[
