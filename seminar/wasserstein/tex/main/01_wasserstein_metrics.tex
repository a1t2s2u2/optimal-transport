...
\begin{proof}
  ...
  任意に $x_0\in\mathcal{X}$ を固定する．三角不等式と距離の対称性より
  \[
    d(x,y)
    \leq d(x,x_0)+d(x_0,y)
    =d(x,x_0)+d(y,x_0).
  \]
  したがって，$t\mapsto t^p$ の凸性
  （注意~\ref{rem:meas-real-facts}）より
  \[
    d(x,y)^p
    \leq
    2^{p-1}\bigl(d(x,x_0)^p+d(y,x_0)^p\bigr).
  \]
  したがって，周辺積分公式（補題~\ref{lem:meas-marginal-integral}）より
  \[
    \begin{aligned}
      J(\mu\otimes\nu)
      &\leq
    ...
    \end{aligned}
  \]
  最後の不等式は $\mu,\nu\in\Pp{p}(\mathcal{X})$ による．よって
  \[
    0\leq I\leq J(\mu\otimes\nu)<\infty,
    \qquad
    I=\Wass_p(\mu,\nu)^p.
  \]
  ...
  \]
  となる $\pi_k$ を取ることができる．したがって
  \[
    J(\pi_k)\longrightarrow I.
  \]

  \textbf{2. 最小化列から弱収束部分列を取る．}
  一点集合は弱位相（定義~\ref{def:meas-weak-convergence}）で
  compact だから，Prokhorov の定理
  （定理~\ref{thm:meas-prokhorov}）より
  $\{\mu\},\{\nu\}$ は tight である．
