...
\begin{proof}
  ...
  \[
    \begin{gathered}
      K\subset\mathcal{X},\quad L\subset\mathcal{Y},
      \qquad K,L\text{ は compact},\\
      \mu(K^c)\leq\frac{\varepsilon}{2}\quad(\forall\mu\in A),
      \qquad
      \nu(L^c)\leq\frac{\varepsilon}{2}\quad(\forall\nu\in B)
    \end{gathered}
  \]
  となる $K,L$ が存在する．任意の $\pi\in\Couplings(A,B)$ に対し，
  \[
    \exists(\mu,\nu)\in A\times B
  ...
  \[
    \begin{aligned}
      ...
      &=\pi\bigl((K^c\times\mathcal{Y})
        \cup(\mathcal{X}\times L^c)\bigr)\\
      &\leq\pi(K^c\times\mathcal{Y})+\pi(\mathcal{X}\times L^c)\\
      &=\mu(K^c)+\nu(L^c)\\
      &\leq\varepsilon.
    \end{aligned}
  \]
  $K\times L$ は compact であり，$\pi$ は任意だから，
  \[
    \pi(K\times L)\geq1-\varepsilon
    \qquad(\forall\pi\in\Couplings(A,B)).
  \]
  よって $\Couplings(A,B)$ は tight である．
\end{proof}
