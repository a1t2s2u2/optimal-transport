...
\begin{proof}
  ...
  したがって $\pi_{k_j}\weakto\pi^*$ と弱収束の定義より
  \[
    \int_{\mathcal{X}\times\mathcal{X}}
...
\begin{proof}
  発想は，共通の中間点 $y\sim\nu$ を先に取り，そのもとで
  $x$ と $z$ を条件付き独立に取ることである．
  測度分解定理（定理~\ref{thm:meas-disintegration}）を
  $\pi,\sigma$ に適用すると
  （後者には第1，第2座標を入れ替えて適用），Markov 核
  $K_\pi,K_\sigma$ が存在して
  \[
