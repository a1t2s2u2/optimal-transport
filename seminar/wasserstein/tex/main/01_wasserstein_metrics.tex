...
\begin{theorem}{$\Wass_p$ の最適 coupling の存在（Villani Thm 4.1）}{wp-optimal-coupling-existence}
  ...
  \]
  この最小値を達成する $\pi^*$ を
  $\Wass_p$ の\textbf{最適 coupling}という．
\end{theorem}
...
\begin{proof}
  ...
  一点集合は任意の位相で compact だから，$\{\mu\},\{\nu\}$ は
  弱位相（定義~\ref{def:meas-weak-convergence}）で compact である．
  したがって Prokhorov の定理（定理~\ref{thm:meas-prokhorov}）と
  補題~\ref{lem:wp-plans-tight}より
  \[
    \{\mu\},\{\nu\}\text{ は tight}
  ...
  再び Prokhorov の定理を適用すると，ある部分列 $(\pi_{k_j})$ と
  ...
  すなわち
  \[
    \int F\,\d\pi_{k_j}\longrightarrow\int F\,\d\pi^*
    \qquad
  ...
  \[
    ...
    =\lim_{j\to\infty}\int\varphi(x)\,\d\pi_{k_j}
    =\int\varphi\,\d\mu.
  \]
  有界連続関数による測度の判定
