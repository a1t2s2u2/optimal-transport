...
最適輸送 (Optimal Transport; OT) とは，
「ある確率分布 $\mu$ を別の確率分布 $\nu$ に変換する，
最もコストの低い方法を見つけよ」という問題である．
...
最適輸送が多くの分野で注目される理由は，
それが確率分布間の距離として
他の指標にはない数学的に優れた性質を持つことにある．

\paragraph{(1) 空間の計量構造を反映する．}
$f$-ダイバージェンス（KL ダイバージェンスなど）は
密度関数の比のみに依存するため，測度の台の位置情報を利用しない．
測度の台が互いに素な場合，KL ダイバージェンスは常に $+\infty$ となり，
分布間の遠近を識別できない．
一方，Wasserstein 距離はコスト関数 $c(x, y)$ を通じて
空間の計量構造を直接取り込む．
...
$W_1(\mu, \nu_2) = 2$ となり，
$\nu_2$ がより遠い分布であるという事実を正しく捉える．
\end{example}

\paragraph{(2) 弱収束を距離付けする．}
適切な可積分条件のもとで，
確率測度列 $(\mu_n)_{n \geq 1}$ が $\mathcal{W}_p$ の意味で $\mu$ に収束することは，
$\mu_n$ が $\mu$ に弱収束することと同値である．
この性質は $f$-ダイバージェンスや全変動距離では一般に成立しない．

\paragraph{(3) 確率測度の空間に幾何構造を与える．}
Wasserstein 距離により，確率測度の空間 $\mathcal{P}_p(\mathcal{X})$ は
計量空間をなす．
この空間では，2 測度の補間（displacement interpolation）や
Fréchet 平均（Wasserstein 重心）が自然に定義され，
測度に対する幾何学的操作の基盤となる（第~\ref{chap:dynamic}章，第~\ref{chap:variational}章）．
