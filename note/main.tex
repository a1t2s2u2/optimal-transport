...
$X=\{1,2,3\}$，$Y=\{a,b\}$ とし，
$\mu(\{1\})=0.5$，$\mu(\{2\})=0.3$，$\mu(\{3\})=0.2$，
$\nu(\{a\})=0.8$，$\nu(\{b\})=0.2$ とする．
コスト関数を
$c(1,a)=1$，$c(2,a)=2$，$c(3,b)=3$，$c(1,b)=4$，$c(2,b)=5$，$c(3,a)=6$
とする．

写像 $T_1(1)=a$，$T_1(2)=a$，$T_1(3)=b$ を考えると，
\begin{align*}
  T_{1\#}\mu(\{a\}) &= \mu(\{1,2\}) = 0.8 = \nu(\{a\}), \\
  T_{1\#}\mu(\{b\}) &= \mu(\{3\}) = 0.2 = \nu(\{b\})
\end{align*}
より $T_{1\#}\mu=\nu$ が成り立ち，$T_1$ は輸送写像の制約を満たす．
このときの輸送コストは
$\int c(x,T_1(x))\,d\mu = 0.5\cdot 1 + 0.3\cdot 2 + 0.2\cdot 3 = 1.7$
である．

一方，$T_2(1)=a$，$T_2(2)=b$，$T_2(3)=a$ とすると，
$T_{2\#}\mu(\{a\})=\mu(\{1,3\})=0.7\neq 0.8=\nu(\{a\})$ となり，
